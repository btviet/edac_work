...
import numpy as np
import pandas as pd
from edac_work.sweet_code.sweet.detect_sw_events import (
...
from edac_work.sweet_code.sweet.detrend_edac import read_detrended_rates
...
from edac_work.sweet_code.read_data.read_from_database import (
...
from edac_work.sweet_code.instruments.read_mex_aspera_data import (
    read_mex_ima_bg_counts, 
    clean_up_mex_ima_bg_counts,
    read_aspera_sw_moments)
from edac_work.sweet_code.instruments.read_msl_rad_data import read_msl_rad_doses, read_msl_rad_filtered_e_doses
...
def ellen_bestilling(startdate, enddate):
    ...
    ax1.axvline(x=datetime.strptime("2021-10-28", "%Y-%m-%d"),
                linestyle='dashed',
                color='black')
    ax2.axvline(x=datetime.strptime("2021-10-28", "%Y-%m-%d"),
                linestyle='dashed',
                color='black')

    ax2.plot(df["date"], df["daily_rate"], marker="o",
             label="EDAC count rate", color="#48b8f3")
    ax2.xaxis.set_major_locator(mdates.DayLocator(3))
    ...
    fig.suptitle("MEX EDAC during Oct. 2021 event ", fontsize=FONTSIZE_TITLE)
    fig.subplots_adjust(top=0.93, hspace=0.1)

    plt.savefig('ellen_mex_edac_oct_2021.png',
                dpi=300, transparent=False,
...
def plot_msl_rad_sweet(start_date, end_date):
    ...
    major_ticks_locations =  [pd.to_datetime('2024-05-20 00:00:00') + pd.Timedelta(days=2 * i) 
                              for i in range(-1, 3)]  # One week away, up to five weeks
    ax2.set_xticks(major_ticks_locations)
...
def plot_maven_sep_fluxes_data_heatmap(filename):
    df = read_maven_sep_flux_data(filename)
    current_date = datetime.strptime("2014-12-19", "%Y-%m-%d")
    ...
    #df = df[df['datetime']>=start_date]df = df[(df['datetime']>=start_date) & (df['datetime'] <= end_date)]
    ...
    plt.savefig('aurora_attempt_8.png',
                 dpi=300, transparent=False)
    plt.show()
...
def plot_one_channel_maven_sep_ion_data(filename):
    ...
    end_date = current_date + pd.Timedelta(days=21)
    df = df[(df["datetime"] >= start_date) & (df["datetime"] <= end_date)]
...
def plot_ima_counts_and_sweet_v3(start_date, end_date):
 # df_ima = read_mex_ima_bg_counts()
    ...
    fig.suptitle("MEX EDAC and IMA bg. counts for Jan. 2012 event",
                fontsize=FONTSIZE_TITLE,
                 y=0.95)
...
if __name__ == "__main__":
    ...
    startdate = datetime.strptime("2012-01-23", "%Y-%m-%d")
    enddate =  datetime.strptime("2012-01-31", "%Y-%m-%d")


    #plot_ima_counts_and_sweet_v3(startdate, enddate)

    startdate = datetime.strptime("2006-02-13", "%Y-%m-%d")
    enddate =  datetime.strptime("2006-02-26", "%Y-%m-%d")
    #show_timerange(startdate, enddate)
    
    start_date = datetime.strptime("2024-05-16", "%Y-%m-%d")
    ...
    filename = 'maven_f_flux_hr_dec_2014'
    ...
    startdate = datetime.strptime("2021-10-07", "%Y-%m-%d")
    enddate = datetime.strptime("2021-11-18", "%Y-%m-%d")
    current_date = datetime.strptime("2021-10-28", "%Y-%m-%d")
    startdate = currentdate - pd.Timedelta(days=21)
    enddate = currentdate + pd.Timedelta(days=21)
    #ellen_bestilling(startdate, enddate)
    


    current_date = datetime.strptime("2021-07-17", "%Y-%m-%d")
    startdate = current_date - pd.Timedelta(days=21)
    enddate = current_date + pd.Timedelta(days=21)
    #ellen_bestilling_2(startdate, enddate) 
    currentdate = datetime.strptime("2021-09-17", "%Y-%m-%d")
    startdate = currentdate - pd.Timedelta(days=21)
    enddate = currentdate + pd.Timedelta(days=21)
    #ellen_bestilling_3(startdate, enddate)
    #show_timerange_counter_countrate(start_date, end_date)
...
def ellen_bestilling_2(startdate, enddate):
    FONTSIZE_AXES_LABELS = 16
    FONTSIZE_AXES_TICKS = 14
    FONTSIZE_TITLE = 20
    FONTSIZE_LEGENDS = 12
    ...
    ax1.axvline(x=datetime.strptime("2021-07-17", "%Y-%m-%d"),
                linestyle='dashed',
                color='black')
    ax2.axvline(x=datetime.strptime("2021-07-17", "%Y-%m-%d"),
                linestyle='dashed',
                color='black')
    ...
    major_ticks_locations = [
        pd.to_datetime('2021-07-17 00:00:00')
        + pd.Timedelta(days=7* i)
    for i in range(-3, 4)
    ]
    ...
    ax1.legend(fontsize=FONTSIZE_LEGENDS)
    ax2.legend(fontsize=FONTSIZE_LEGENDS, loc='upper left')
    fig.suptitle("MEX EDAC during July 2021 event ", fontsize=FONTSIZE_TITLE)
    fig.subplots_adjust(top=0.93, hspace=0.1)

    plt.savefig('ellen_mex_edac_july_2021.png',
                dpi=300, transparent=False,
                 bbox_inches="tight", pad_inches=0.05)
    
    plt.show()

def ellen_bestilling_3(startdate, enddate):
    FONTSIZE_AXES_LABELS = 16
    FONTSIZE_AXES_TICKS = 14
    FONTSIZE_TITLE = 20
    FONTSIZE_LEGENDS = 12
    ...
    ax1.axvline(x=datetime.strptime("2021-09-17", "%Y-%m-%d"),
                linestyle='dashed',
                color='black')
    ax2.axvline(x=datetime.strptime("2021-09-17", "%Y-%m-%d"),
                linestyle='dashed',
                color='black')
    ...
    major_ticks_locations = [
        pd.to_datetime('2021-09-17 00:00:00')
        + pd.Timedelta(days=7* i)
    for i in range(-3, 4)
    ]
    ...
    ax1.legend(fontsize=FONTSIZE_LEGENDS)
    ax2.legend(fontsize=FONTSIZE_LEGENDS, loc='upper left')
    fig.suptitle("MEX EDAC during Sept. 2021 event ", fontsize=FONTSIZE_TITLE)
    fig.subplots_adjust(top=0.93, hspace=0.1)

    plt.savefig('ellen_mex_edac_sept_2021.png',
                dpi=300, transparent=False,
                 bbox_inches="tight", pad_inches=0.05)
